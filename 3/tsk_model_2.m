...
% Training Hybrid Algorithm. 3 MF. Singleton output.
% initialize 
initFis = genfis1(d_trn, 3, 'gbellmf', 'constant');
...
saveas(gcf, 'MembershipFunctionsBeforeTraining2.png');

%training for backpropagation (1 in anfis means hybrid)
nEpochs = 1000;
trnOpt = [nEpochs, NaN, NaN, NaN, NaN];
disOpt = [0, 0, 0, 0];
[trnFis, trnError2, stepsize, chkFis, chkError2 ] = anfis(d_trn, initFis,  trnOpt, disOpt, d_val, 1);
...
saveas(gcf, 'MembershipFunctionsAfterTraining2.png');
...
%plot errors
figure(4)
plot([trnError2 chkError2])
hold on
plot([trnError2 chkError2], 'o')
legend('trnError', 'chkError')
xlabel('Epochs')
ylabel('RMSE (Root Mean Squared Error)' )
title('Error Curves')
saveas(gcf, 'ErrorCurves2.png');
...
saveas(gcf, 'ComparisonBetweenRealPredictedValues2.png');