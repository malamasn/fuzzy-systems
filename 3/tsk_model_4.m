...
% Training Hybrid Algorithm. 3 MF. Polynomial output.
% initialize 
initFis = genfis1(d_trn, 3, 'gbellmf', 'linear');
...
saveas(gcf, 'MembershipFunctionsBeforeTraining4.png');

%training for backpropagation (1 in anfis means hybrid)
nEpochs = 500;
trnOpt = [nEpochs, NaN, NaN, NaN, NaN];
disOpt = [0, 0, 0, 0];
[trnFis, trnError4, stepsize, chkFis, chkError4 ] = anfis(d_trn, initFis,  trnOpt, disOpt, d_val, 1);
...
saveas(gcf, 'MembershipFunctionsAfterTraining4.png');
...
%plot errors
figure(4)
plot([trnError4 chkError4])
hold on
plot([trnError4 chkError4], 'o')
legend('trnError', 'chkError')
xlabel('Epochs')
ylabel('RMSE (Root Mean Squared Error)')
title('Error Curves')
saveas(gcf, 'ErrorCurves4.png');
...
saveas(gcf, 'ComparisonBetweenRealPredictedValues4.png');