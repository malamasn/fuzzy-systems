...
% Training Hybrid Algorithm. 2 MF. Polynomial output.
% initialize 
initFis = genfis1(d_trn, 2, 'gbellmf', 'linear');
...
saveas(gcf, 'MembershipFunctionsBeforeTraining3.png');
...
disOpt = [0, 0, 0, 0];
[trnFis, trnError3, stepsize, chkFis, chkError3 ] = anfis(d_trn, initFis,  trnOpt, disOpt, d_val, 1);
...
saveas(gcf, 'MembershipFunctionsAfterTraining3.png');
...
%plot errors
figure(4)
plot([trnError3 chkError3])
hold on
plot([trnError3 chkError3], 'o')
legend('trnError', 'chkError')
xlabel('Epochs')
ylabel('RMSE (Root Mean Squared Error)' )
title('Error Curves')
saveas(gcf, 'ErrorCurves3.png');
...
saveas(gcf, 'ComparisonBetweenRealPredictedValues3.png');